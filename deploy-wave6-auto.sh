#!/bin/bash
# Auto deploy Wave 6 to VPS with password

VPS_HOST="159.65.134.137"
VPS_USER="root"

echo "🔐 Enter VPS password:"
read -s VPS_PASS

echo ""
echo "🚀 Deploying Wave 6 to VPS..."

# Create multichain_config.py
echo "📤 Uploading multichain_config.py..."
sshpass -p "$VPS_PASS" ssh -o StrictHostKeyChecking=no $VPS_USER@$VPS_HOST "cat > /root/Aura-Protocol-V.1-main/backend/multichain_config.py" << 'EOF'
...
# Create multichain_routes.py
echo "📤 Uploading multichain_routes.py..."
sshpass -p "$VPS_PASS" ssh -o StrictHostKeyChecking=no $VPS_USER@$VPS_HOST "cat > /root/Aura-Protocol-V.1-main/backend/multichain_routes.py" << 'EOF'
...
# Check if multichain routes already added
if ! grep -q "multichain_routes" server.py; then
    # Find line with DeFi routes and add after it
    sed -i '/from defi_routes import router as defi_router/a\
\
...
try:\
    ...
    echo "⚠️ multichain_routes already in server.py"
fi
ENDSSH

# Restart backend
echo "🔄 Restarting backend..."
sshpass -p "$VPS_PASS" ssh -o StrictHostKeyChecking=no $VPS_USER@$VPS_HOST "systemctl restart aura-backend-stable"

sleep 3

# Check status
echo "📊 Checking status..."
sshpass -p "$VPS_PASS" ssh -o StrictHostKeyChecking=no $VPS_USER@$VPS_HOST "systemctl status aura-backend-stable --no-pager | head -20"

echo ""
echo "✅ Wave 6 deployment complete!"
echo "🔗 Test endpoints:"
echo "   curl https://api.aurapass.xyz/api/networks"
echo "   curl https://api.aurapass.xyz/api/contracts/SimpleZKBadge"
